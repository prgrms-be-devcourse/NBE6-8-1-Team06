...
useEffect(() => {
  async function fetchProduct() {
    try {
      ...
      console.log('상품 상세 응답:', json); // 여기가 가장 중요!
        ...
        <div className="flex gap-6 border p-6 rounded shadow hover:shadow-md transition w-full max-w-2xl">
          ...
          <div className="flex flex-col justify-between items-end">
            <div className="flex items-center gap-2 mb-4">
              <button onClick={() => setQuantity((prev) => Math.max(prev - 1, 1))} className="px-3 py-1 bg-gray-300 rounded">-</button>
              <span className="min-w-[30px] text-center font-semibold">{quantity}</span>
              <button onClick={() => setQuantity((prev) => prev + 1)} className="px-3 py-1 bg-gray-300 rounded">+</button>
            </div>
